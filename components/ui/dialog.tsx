  ...
  return (
    <DialogPortal data-slot="dialog-portal">
      ...
      <DialogPrimitive.Content
        ...
        className={cn(
          "bg-card data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 fixed top-[50%] left-[50%] z-50 flex flex-col w-full max-w-[calc(100%-2rem)] max-h-[calc(100vh-2rem)] max-h-[calc(100dvh-2rem)] translate-x-[-50%] translate-y-[-50%] rounded-lg border shadow-lg duration-200 outline-none sm:max-w-lg overflow-hidden",
          className
        )}
        {...props}
      >
        <div className="flex flex-col min-h-0 flex-1 p-6 overflow-y-auto">
          {children}
        </div>
        {showCloseButton && (
...
function DialogFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex flex-col-reverse gap-2 sm:flex-row sm:justify-end flex-shrink-0 pt-4",
        className
      )}
      {...props}
    />
